...
* Because of the license incompatibility bewteen Crate Digger and
Remote Tea, that is probably going to be moved out into a separate
library which can be separately linked by client programs that need
to obtain `.pdb` file and related resources over the network, and
which can be substituted for alternate implementations using different
mechanism or available under different licenses as needed.
