...
\begin{figure}
  \begin{bytefield}[bitwidth=1.9em, leftcurly=., leftcurlyspace=0pt, boxformatting={\baselinealign}]{16}
    ...
    \end{leftwordgroup} \\
    \begin{leftwordgroup}{\tiny\bfseries 10}
      \colorbitbox{yellow}{4}{$len\_cues$} & \colorbitbox{yellow}{4}{$memory\_count$} & \bitbox[lrt]{8}{Cue Entries}
    \end{leftwordgroup} \\
    \begin{leftwordgroup}{}
      \skippedwords \\
      \wordbox[lrb]{1}{}
...
This kind of section is used only in rekordbox's Performance Mode, and
...
{\tt 18}. Many thanks to Michael Ganss,
...
\begin{figure}
  \begin{bytefield}[bitwidth=1.9em, leftcurly=., leftcurlyspace=0pt, boxformatting={\baselinealign}]{16}
    ...
    \begin{leftwordgroup}{\tiny\bfseries 10}
      ...
      \colorbitbox{yellow}{2}{$end$} & \bitbox[lrt]{4}{$entries$}
    \end{leftwordgroup} \\
    \begin{leftwordgroup}{}
      \skippedwords \\
      \wordbox[lrb]{1}{}
...
up; for this kind of tag it always has the value 24. $len\_entries$ at
bytes~{\tt 10-11} (labeled $len_e$ in
Figure~\ref{fig:songStructureTagStructure}) specifies how many entries
are present in the tag. Each entry represents one recognized phrase.
$phrase\_style$ at bytes~{\tt 12-13} (labeled $phrase$) specifies the
overall type of phrase structure that rekordbox chose to represent the
song. The value $1$ is an ``up-down'' style where the main phrases
consist of ``up'', ``down'', and ``chorus'', and is represented in
rekordbox by white label text. The value $2$ is a ``bridge-verse''
style where the main phrasesare labeled ``verse'', ``chorus'', and
``bridge'', and the label text in rekordbox is black. Style $3$ is
mostly identical to bridge-verse, except verses 1--3 are labeled
``VERSE1'' and verses 4--6 are labeled ``VERSE2'' in rekordbox.
...
The phrase entries begin at byte~{\tt 1c}, and
...
of $phrase\_style$ in the tag header, as is detailed in the table
below.
...
TODO: Add phrase ID table here, and make actual reference to it above.
