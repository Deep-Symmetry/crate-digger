...
Many row types store string values, by storing an offset to the
strange structure described in this section. The strings can be stored
in a variety of formats. The first byte of the structure, labeled
$length\_and\_kind$ in the parsed Kaitai Struct, identifies the
encoding type and, when it is odd, also the length (for short ASCII
strings), as detailed in Section~\ref{sec:shortAscii}.
...
$mangled\_length$, divide it by two, and subtract 1 again.
