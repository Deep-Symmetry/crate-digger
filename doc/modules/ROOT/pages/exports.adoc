...
Bytes{nbsp}``0``-`1` are called __transaction_row_count__ (abbreviated _tran~rc~_ in the diagram), and store the number of rows touched in the last transaction on this page.
Bytes{nbsp}``2``-`3` are called __transaction_row_index__ (abbreviated _tran~ri~_ in the diagram), and store the index of the first row touched in the last transaction on this page.
...
Once there have been sixteen rows added, all the bits in _row~pf0~_ are accounted for, and when another row is added, before its offset entry _ofs~16~_ can be added, another row bit-mask entry _row~pf1~_ needs to be allocated, followed by its corresponding _tran~rf1~_.
...
Index pages have a __page_flags__ value of `0x64`. They follow the common header with an index-specific header and a list of index entries. These entries seem to point to a page which has deleted rows, which would make sense in order to provide a way to find free space when inserting new rows.
...
The first two bytes are called _unknown~a~_ and they are usually `0x1fff` or `0x0001`, although values like `0x0002` have been observed on tables with a big number of rows.

After that, bytes `2-3` are called _unkown~b~_ and they're usually `0x1fff` or `0x0000`, although values like `0x00b3` have been observed on tables with a big number of rows.

Next comes a 2-byte magic value `0x03ec`. 
...
The two following 32-bit fields contain another magic value `0x03ffffff` and zeros `0x00000000`.

The __num_entries__ field at bytes `24-25` (abbreviated _num~e~_) indicates how many index entries are present in the page. Field __first_empty__ at bytes `26-27` (abbreviated _first~e~_) points to the first empty index entry, and is `0x1fff` if there are none. An empty index entry has the value `0x1ffffff8`.
...
Bits `31-3` are a page index (i.e. the page that is pointed to by this index entry) but without the 3 most significant bits. Bits `0-2` (called `index_flags`) have unknown purpose, and their most common value is just `0x000`.
